...
- ウォームアップ終了時にLocustの累積統計をリセットし、立ち上がり区間をp95・p99・エラー率から分離
- 試験プロセスの実行成否と、性能基準のPASS／FAILを分離
...
3. CSVのp95・p99・エラー率・スループットをPythonで評価する
...
`run_scenario.py`は同じ設定値を`POC_WARMUP_SECONDS`としてLocustへ渡します。Locustは試験開始イベントを受け取り、指定秒数後に`runner.stats.reset_all()`を実行します。

これにより、ウォームアップ中のリクエスト、失敗数、応答時間の累積値がリセットされ、その後の定常区間を新しい測定期間として記録します。
分析スクリプトは、CSV内の累積リクエスト数が途中で減少したことを確認し、統計リセットの実績をレポートします。
...
応答時間が悪化すると、同じユーザー数で試験していても単位時間あたりに処理できるリクエスト数が低下する場合があります。このPoCでは、ウォームアップ区間を除外した後のRPS中央値をbaselineと比較します。

```text
RPS低下率 =
（baselineのRPS中央値 - 比較対象のRPS中央値）
÷ baselineのRPS中央値 × 100
```

`config/thresholds.json`の`regression_fail.rps_decrease_percent_max`を許容値として使用し、現在はPoC用の暫定値として20%に設定しています。

例えば、baselineが100 requests/s、比較対象が75 requests/sの場合、低下率は25%です。許容値20%を超えるため、回帰判定は`FAIL`になります。

RPSは実行環境、負荷生成側の性能、ネットワーク、待機時間などの影響も受けます。そのため、RPS低下だけでアプリケーションを原因と断定せず、manifestによる比較条件の一致と追加の監視情報を確認します。

## 6. 判定の責任分界

本PoCでは、性能試験におけるAIの役割を「分析支援」に限定しています。

### Python
- p95 / p99 / エラー率 / RPSなどの数値計算
- 閾値との比較
- baseline / regression比較
- PASS / FAIL判定
...
本PoCで使用する閾値は、判定ロジックを検証するための暫定値です。
実案件ではSLA/SLO、利用モデル、ピーク負荷、業務要件に基づいて決定します。

応答性能とエラー率は利用者影響に直結するため`FAIL`、CPU・メモリは単独で不合格理由にせず`WARN`とします。

### 実行成否と性能判定

Locustには`--exit-code-on-error 0`を指定し、意図的なHTTP 503が発生しても試験プロセス自体は正常終了させます。これにより、次の2つを分離します。

| 判定 | 意味 | 判定主体 |
|---|---|---|
| 実行ステータス | 負荷試験が最後まで実行できたか | runner |
| 性能判定 | 応答時間・エラー率・RPSが基準内か | 分析スクリプト |

例えば、試験が最後まで完走して性能基準を超過した場合、manifestは`COMPLETED`、性能レポートは`FAIL`になります。
...
- シナリオ時間にウォームアップを含めるか、実測時間へ加算するか
- Locustを単体実行するか、master／workerによる分散実行にするか
